...
for FILE in `egrep -v '(^#|^$)' proprietary-files.txt`; do
  ...
  if [[ ! "$FILE" =~ ^-.* ]]; then
    ...
    if [ -e "../../../$OUTDIR/proprietary/$FILE" ]; then
...
LINEEND=" \\"
COUNT=`wc -l ../miata/proprietary-files.txt | awk {'print $1'}`
DISM=`egrep -c '(^#|^$)' ../miata/proprietary-files.txt`
COUNT=`expr $COUNT - $DISM`
for FILE in `egrep -v '(^#|^$)' ../miata/proprietary-files.txt`; do
  COUNT=`expr $COUNT - 1`
  if [ $COUNT = "0" ]; then
    LINEEND=""
  fi
